/*
$Id: make_binary_static.cc, last modified 2010/10/10 22:55:01 collignon Exp $
...
//  DEFINE HDF5 VARIABLE1
    ...
    hsize_t dimsfb[1],dimsfbb[2],dimsfcc[2],dimsfc[1];
    herr_t status,ret;
...
//      COUNT THE NUMBER OF LINE IN THE MAP FILE
        ...
        char buf_head[6][LINE_LEN];
        char buf_fld[5][LINE_LEN];
...
//            printf("before create dset6:%s\n",buf_name6);
            sprintf (buf_nameX,"%s%s","Total","nummap");
//            printf("before create dset7:%s\n",buf_name7);
        } else if (strcmp(indicMap,"fld") == 0) {
...
//      status = H5Fclose(file_map);
    ...
    printf("MAP_TOT:%d\n",MAP_TOT);
    int tab_map_tot[1];
    tab_map_tot[0]=MAP_TOT;
    string_dset = H5LTmake_dataset(file_map,buf_nameX,1,dimsfc,H5T_NATIVE_INT,&tab_map_tot[0]);
